...
(define (elsamuko-national-geographic-batch shadowopacity
                                            sharpness screenopacity
                                            overlayopacity localcontrast
                                            screenmask tint pattern)
  (let* ((filelist (cadr (file-glob pattern 1))))
    (while (not (null? filelist))
           (let* ((filename (car filelist))
             ...
             (gimp-file-save RUN-NONINTERACTIVE img adraw (string-append (car (strbreakup filename ".")) "_ng.jpg") (string-append (car (strbreakup filename ".")) "_ng.jpg"))
...
(script-fu-register "elsamuko-national-geographic-batch"
                    _"_National Geographic Batch ..."
...
Latest version can be downloaded from http://registry.gimp.org/node/9592"
                    ...
                    "22/09/08"
                    "*"
                    SF-ADJUSTMENT _"Shadow Recover Opacity"   '(50  0  100  1   5 0 0)
...
(script-fu-menu-register "elsamuko-national-geographic-batch" _"<Toolbox>/_Filters/_Script-Fu")